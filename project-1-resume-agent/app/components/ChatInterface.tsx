...
import { Send, User, Bot, Sparkles, Paperclip, ChevronRight, Loader2, Github, ExternalLink } from "lucide-react";
...
export default function ChatInterface() {
    ...
    return (
        <div className="flex flex-col items-center gap-6 w-full max-w-4xl mx-auto">
            <a
                href="https://github.com/vybhav-bhadri/agentic-ai"
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-full text-sm text-gray-300 transition-all hover:scale-105 cursor-pointer backdrop-blur-md group"
            >
                <Github className="w-4 h-4 text-white/70 group-hover:text-white transition-colors" />
                <span>Check out more projects on GitHub</span>
                <ExternalLink className="w-3 h-3 opacity-50 group-hover:opacity-100 transition-opacity" />
            </a>

            <div className="flex flex-col h-[65vh] max-h-[800px] w-full bg-white/10 backdrop-blur-lg rounded-3xl shadow-2xl overflow-hidden border border-white/20">
                {/* Header */}
